...
function compileTableSql(code, path) {
    ...
    return `
  publish("${getEscapedFileName(path)}").query(ctx => {
...
function compileOperationSql(code, path) {
    ...
    return `
  operate("${getEscapedFileName(path)}").queries(ctx => {
    ${functionsBindings.join("\n")}
    ${js}
    return \`${sql}\`.split("\\n---\\n");
  })`;
...
function compileAssertionSql(code, path) {
    ...
    return `
  assert("${getEscapedFileName(path)}").query(ctx => {
    ${functionsBindings.join("\n")}
    ${js}
    return \`${sql}\`;
  })`;
